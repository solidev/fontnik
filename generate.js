...
if (process.argv.length < 4) {
  console.log("USAGE: node generate.js INPUT_DIR OUTPUT_DIR")
}
...
var todo = [];
fs.readdirSync(inputDir).forEach(function(dir) {
  console.log("Reading", inputDir, dir);
  if (fs.lstatSync(path.join(inputDir, dir)).isDirectory()) {
    var fonts;
    try {
      ...
      fonts = [];
      fs.readdirSync(path.join(inputDir, dir)).forEach(function(file) {
    ...
    if (fonts && fonts.length) {
      todo.push([path.join(inputDir, dir), fonts]);
    }
  }
});
